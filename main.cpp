...
int direction2 =10;    //direction the snake is moving
...
void move(){
    ...
    if (((s[0].x) == food.x) && ((s[0].y) == food.y))
    {
        ...
        snake_length++;
        if(delay>0.02f){
              delay -= 0.01; //if snake eat fruit, make game faster
        }
...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::W)) direction2 = 10;
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::S)) direction2 = 11;
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::A)) direction2 = 12;
        if (sf::Keyboard::isKeyPressed(sf::Keyboard::D)) direction2 = 13;
        if (timer > delay)
        {
            timer = 0; //reset timer
